...
-(void)openDatabase
{
    ...
    NSString *desPath=[[[paths objectAtIndex:0] stringByAppendingFormat:@"/Caches"]  stringByAppendingPathComponent:@"myCache.db"];
...
-(void)createTable
{
    NSString *createSQL = @"CREATE TABLE IF NOT EXISTS CACHE (ROW INTEGER PRIMARY KEY, TYPEID INTEGER, MAXLEVEL INTEGER, MINLEVEL INTEGER,NAME TEXT,LAYERNAME TEXT,RANGE TEXT, ISSHOW INTEGER);";
...
-(void)insertCache:(DBCache *)theCache{
    if(theCache == NULL)
        return;
    NSString *update = @"INSERT OR REPLACE INTO CACHE (TYPEID,MAXLEVEL,MINLEVEL,NAME,LAYERNAME,RANGE,ISSHOW) VALUES (?,?,?,?,?,?,?);";
    BOOL bSecceed = [database executeUpdate:update,[NSNumber numberWithLong:theCache.typeID],[NSNumber numberWithLong:theCache.maxLevel],[NSNumber numberWithLong:theCache.minLevel],theCache.name,theCache.layerName,theCache.range,[NSNumber numberWithLong:theCache.isShow]];
    if (!bSecceed)
    {
...
- (void)updateCache:(DBCache *)theCache{
    if(theCache == NULL)
        return;
    NSString *update = @"update CACHE set NAME = ?,MAXLEVEL = ?,MINLEVEL = ?,LAYERNAME = ? ,RANGE = ? ,ISSHOW = ? WHERE TYPEID =";
    ...
    BOOL bSucceed = [database executeUpdate:update,
                     theCache.name,[NSNumber numberWithLong:theCache.maxLevel],[NSNumber numberWithLong:theCache.minLevel],theCache.layerName,theCache.range,[NSNumber numberWithLong:theCache.isShow]];
...
-(NSMutableArray *)getAllCache{
    ...
    NSString *query = @"SELECT TYPEID,MAXLEVEL,MINLEVEL,NAME,LAYERNAME,RANGE,ISSHOW FROM CACHE;";
